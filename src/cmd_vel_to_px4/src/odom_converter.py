...
class OdomConverter(Node):
    def __init__(self):
        ...
        self.world_transform = np.array([
            [1, 0, 0],  # X_enu = Y_ned (East)
            [0, -1, 0],  # Y_enu = X_ned (North)
    ...
    def transform_orientation(self, px4_quaternion):
        """Transform orientation from PX4 NED to ROS ENU."""
        w = float(px4_quaternion[0])
        x = float(px4_quaternion[1])
        y = float(px4_quaternion[2])
        z = float(px4_quaternion[3])
        ...
        return [float(q_x), float(q_y), float(q_z), float(q_w)]

    def listener_callback(self, msg: VehicleOdometry):
        # self.get_logger().info(f"Received odometry: pos=({msg.position[0]:.2f}, {msg.position[1]:.2f}, {msg.position[2]:.2f})")
